...
\emph{sign}, \emph{exponent}, and \emph{significand} fields specify how many
bits are allocated to encode the sign of the number, its scale, and its
significant digits, respectively. The \emph{exponent bias} is a constant added
to the true exponent so that it can be stored as an unsigned integer. The
\emph{precision} (in bits) and corresponding \emph{decimal digits} tell how
