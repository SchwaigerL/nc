...
\noindent
AMP improves deep learning training by using both FP16 and FP32 data types
reducing memory usage and increasing computational speed. However, FP16's
limited dynamic range can cause numerical instability, especially during
backpropagation. To address this, Gradient Clipping is used to cap the
magnitude of gradients (either globally or per element) preventing extreme
updates that could hinder convergence. Together, AMP and gradient clipping
enable efficient, stable training by balancing performance and numerical
precision~\cite{micikevicius2017mixed}.
...
\begin{equation}\label{eq:cegrads}
	\frac{\partial L}{\partial z_k}=p_k-\mathbbm{1}_{k=y},\qquad\frac{\partial L}{\partial W}=(p-e_y)h^\top,\qquad\frac{\partial L}{\partial b}=p-e_y,
\end{equation}
...
$\mathbbm{1}_{k=y}$ is the indicator: it equals \num{1} if $k=y$ and \num{0}
otherwise. For evaluation, the model is fed multiple inputs and the results of
the indicator function are summed up and divided by the total number of inputs.
This is then called the accuracy score of the model which can be understood as
...
\begin{table}[H]
	...
	\begin{tabular}{@{}lcccc@{}}
		...
		\bottomrule
	\end{tabular}
	\caption{Field widths (in bits) of common IEEE754 formats\protect\footnotemark.}\label{tab:binary-bits}
\end{table}
\footnotetext{\label{footnoteWiki}\url{https://en.wikipedia.org/wiki/Floating-point_arithmetic}}
...
\begin{table}[H]
	...
	\begin{tabular}{@{}lccc@{}}
		...
		\bottomrule
	\end{tabular}
	\caption{Range and precision characteristics of the formats\protect\footnotemark[\getrefnumber{footnoteWiki}].}\label{tab:binary-precision}
\end{table}
...
\noindent
For the MNIST experiments (i) FP64/FP32/FP16 end-to-end training, (ii) effects
on timing, (iii) inference-only casting, and (iv) AMP with gradient clipping,
we use 5-fold cross-validation on the training set with random equal splits.
Unless noted otherwise, figures report fold means, and no test set tuning is
...
inference-only casting, we first train a baseline model in FP64 and then do
inference on copies cast to FP32 and FP16 without any further optimization (so
no \textit{retraining} on the lower precision). The AMP experiment is run on
MNIST with gradient clipping, comparing an AMP setup (FP16 computations with
FP32 master weights) against a pure FP32 baseline. We try different global-norm
and per-element value-clipping thresholds to check stability and report test
accuracy and training/inference times.\\

\noindent
On the FashionMNIST dataset, experiment (v) is a rounding experiment (batch
size \num{256}, FP32 training) which means that after training only the stored
weights are rounded to $k$ decimal places ($k{=}1\ldots14$) and inference
remains at FP32. Inputs and intermediate computations stay at full precision.
We report test accuracy as a function of $k$ and note the impact on storage.

\section{Experiments and Results}\label{sec:results}
In the first experiment, every floating-point variable that is used inside the
training loop was converted to a lower precision data type. This therefore
includes the weights and biases, activations, gradients, and optimizer states.
...
\begin{figure}[H]
	...
	\caption{Model accuracy evaluated after training with different precisions (5-fold CV).}\label{fig:modelAccTrainAndInf}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Training and evaluation times for the different precisions (5-fold CV).}\label{fig:modelEvalTrainTimes}
\end{figure}
...
This experiment uses AMP with different gradient clipping methods. The first
method is called global norm clipping where all gradients are collectively
scaled to ensure their combined norm does not exceed a specified threshold,
which ensures stable updates across the network. The second method is
per-element clipping which constrains each individual gradient component within
a fixed range, directly limiting extreme values. These methods are evaluated
under AMP to compare their impact on training stability, model accuracy, and
computation time across different precision types.\\

\noindent
Figure~\ref{fig:ampGradientNormClipping} depicts the effect of the
\textit{global} gradient norm on accuracy, training and testing timings.
Accuracy is virtually the same between FP16 and FP32 when gradient clipping is
used validating the stability of AMP and gradient scaling. Eval time is nearly
identical for both formats, as evaluation involves fewer operations and less
intense memory bandwidth usage.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{figures/FP16_32_clipping.png}
	\caption{Effect of global gradient norm clipping on accuracy, training and testing timings.}\label{fig:ampGradientNormClipping}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{figures/gradclip_16.png}
	\caption{Effect of per-element gradient clipping on FP16 $\rightarrow$ Accuracy vs training and evaluation time}\label{fig:grad16}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{figures/gradclip_32.png}
	\caption{Effect of per-element gradient clipping on FP32 $\rightarrow$ Accuracy vs training and evaluation time}\label{fig:grad32}
\end{figure}

\noindent
Figure~\ref{fig:grad16} shows that accuracy peaked at 97.07\% with a clipping
value of 0.5, while the lowest accuracy (96.55\%) occurred at clip=0.01,
suggesting that very small clip values may overly restrict gradient updates and
hurt learning. Training time varied, which indicates that moderate clipping
supports efficient convergence. The evaluation time remained stable across all
clip values (around 2.01s to 2.16s), showing that clipping primarily impacts
training rather than inference performance. Figure~\ref{fig:grad32} shows that
accuracy was most stable across all clip values with the highest accuracy at
clip=0.5, demonstrating its robustness to gradient clipping variations. The
training was conducted with 5-fold CV, 10 epochs, and batch size 64. The
training time suggests that larger clip values can speed up convergence without
sacrificing stability. Evaluation time confirms that clipping had only a minor
...
though, which begs the question if one could reduce the precision of the model
weights. In this section, we explore a naive quantization method: Rounding to a
fixed number of decimals. With a small enough number of decimals and a
reasonable range for the integer part, one could store only the significant and
provide base as well as exponent as separate data. When loading the model, the
weights are converted back to floating point, with whatever precision is
desired. Only the weights are quantized; the input data, as well are the values
propagated through the network are at full precision.\\
...
\noindent
Figure~\ref{fig:roundingDistribution1} shows the distribution of the model
weights in a three layer architecture. The values are in $[-0.5, 0.5]$, which
means no integer part is needed in this example. In Figure~\ref{fig:118_2} the
weights of the same model rounded to two decimals is depicted. The envelope of
the bins is still very similar to the full precision distribution, which makes
it appear like a good approximation. The results of evaluating this heuristic
is shown in Figure 11, where the results of how rounding the decimal part
affects the test accuracy is shown.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{figures/118_dist.png}
	\caption{
		Distribution of model weights in each layer.}\label{fig:roundingDistribution1}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{figures/118_dist2.png}
	\caption{Distribution of model weights in each layer after rounding to two decimals.}\label{fig:118_2}
\end{figure}
...
floating point. Making an educated guess the on distribution such as
...
just \num{10} Bits for representing 3 decimals, no bits for the integer part
and one additional bit for the sign, only \num{11} Bits must be stored to have
the best performing model for inference. The storage size of the model is
reduced to $\frac{11}{32}\approx 34\%$ of the otherwise full \num{32} Bit
model.

\section{Summary and Conclusion}

Regarding the precision reduction during training and inference, models with
16-bit floating-point (FP16) precision preserved accuracy comparable to
higher-precision formats (FP32 and FP64). This suggests that for tasks of
similar complexity, FP16 is sufficient, offering potential memory and
computational savings without sacrificing performance. Reducing precision only
during inference, after training with high-precision (FP64), maintained
baseline accuracy. This is particularly valuable for deployment scenarios where
training is done on powerful hardware, but inference occurs on
resource-constrained devices. Training with mixed precision and Gradient
Clipping resulted in similar test accuracy scores as precision reduction during
training as well as post-training quantization. No significant decrease in
model performance accuracy was detected. The train and test times were similar,
regardless of using FP32, FP16 or a mix of both. Rounding trained weights to
just three decimal digits (approximately 11 bits) achieved full accuracy,
highlighting the potential for aggressive quantization in storage-efficient
model deployment. This finding is significant for edge applications where
