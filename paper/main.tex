...
\begin{abstract}
	...
	Using MNIST as a dataset, we benchmark uniform quantization (mapping real
	values to evenly spaced levels via a single step size), mixed-precision
	formats, and post-training decimal rounding. We evaluate precision settings
	(FP64/FP32/FP16) across all tensors during training, experiment with an
	inference-only quantization path on FP64-trained models, and test automatic
	mixed precision with gradient clipping. Metrics include test accuracy,
