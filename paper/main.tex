...
formats~\cite{zhou2018dorefa}. The method allows using \textit{arbitrary
	bitwidths}, i.e., the number of bits is not restricted to standard IEEE
...
training. An often used approach is integer-only quantization with symmetric
...
minimal or no retraining~\cite{frantar2022gptq}. GPTQ treats quantization as an
optimization problem by using approximations of the local curvature to choose
quantization levels that minimize the error introduced by replacing
...
range supported by low-precision integer formats. This improves the robustness
of post-training quantization, especially in transformer architectures where
...
$\sigma$ is a nonlinear activation function (e.g., sigmoid or ReLU).
...
benefit is that the computation is very fast and simple compared to more
...
$\mathbbm{1}\{k=y\}$ is the indicator: it equals $1$ if $k=y$ and $0$
otherwise.\\

\noindent
Memory and computational requirements during training are substantial, as
weights, gradients, activations, and parameters must be stored and updated
iteratively. Training and inference can both be optimized using precision
reduction and related techniques, but inference is more well suited to
optimizations because it only requires a forward pass.
